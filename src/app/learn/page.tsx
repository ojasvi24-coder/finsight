...
import { ArrowRight, Activity, Database, Server, Cpu, Zap, TrendingUp, Shield, Sparkles, BrainCircuit, TrendingDown, Globe } from "lucide-react";
...
export default function Home() {
  ...
  // NEW: Interactive State for the Homepage Live Preview!
  ...
  // NEW: Dynamically generate chart data based on the user dragging the slider
  const demoProjectionData = useMemo(() => {
    let baseBalance = 25000;
    let optBalance = 25000;
    const monthlySavings = 1200; // Baseline savings assumption for the demo
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-50 selection:bg-emerald-500/30 font-sans overflow-hidden relative">
      ...
      <main className="relative mx-auto max-w-7xl px-6 sm:px-8 pt-20 sm:pt-32 pb-20 z-10">
        ...
        {/* GENUINELY LIVE DASHBOARD PREVIEW */}
        <motion.div initial={{ opacity: 0, y: 60 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 1 }} className="mb-32 mx-auto max-w-6xl relative group">
          ...
          <div className="relative rounded-3xl bg-slate-950/80 backdrop-blur-xl border border-slate-700/50 shadow-2xl overflow-hidden">
            <div className="h-12 bg-slate-900 border-b border-slate-800 flex items-center px-6 gap-3">
              ...
              <div className="flex-1 flex justify-center">
                <div className="bg-slate-950 px-32 py-1 rounded-md text-xs font-mono text-slate-500 border border-slate-800 flex items-center gap-2">
                  <Shield className="h-3 w-3 text-emerald-500" /> app.finsight.ai
                </div>
              </div>
            </div>
            ...
            <div className="p-8 sm:p-12">
              ...
              <div className="mb-8 flex flex-col sm:flex-row justify-between sm:items-end gap-6">
                <div>
                  ...
                  <p className="text-slate-400">Drag the slider to test the FinSight algorithm instantly.</p>
                </div>
                
                {/* NEW: Homepage Interactive Slider */}
              ...
              <div className="bg-slate-900/50 p-6 rounded-2xl border border-slate-800 h-[350px]">
                {/* Chart is now fed by LIVE React State, not static mock data! */}
